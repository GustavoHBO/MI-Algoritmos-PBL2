...
void gerar_relatorio(int paises, int opcao){
    ...
    float porcentagem = 0;
    ler_entrada(matriz);
    fprintf(arquivo, "\t\tQuadro de Medalhas Inicial (Antes da Adição de Medalhas).\n\n");
    ...
    if (opcao == 2)
        ler_matriz(matriz);
    fprintf(arquivo, "\n\n\n\t\tRanking com Desempenho dos Países Ordenado Pela Numeração (ao Encerrar o Programa).\n\n");
    fprintf(arquivo, "\n\tOuro\t|\tPrata\t|\tBronze\t|\tTotal\n\n");
    for ( cont = 0; cont < paises; cont++ ){
        fprintf(arquivo, "[P%d]\t", matriz[cont][4]+1);
        for ( cont2 = 0; cont2 < 4; cont2++ ){
